...
def collapse_into_line(phone_file, j, k):
    ...
    #print("after custom, before encode")
    phone_str = encode_phone(phone)
    #print(phone_str, "no encode")
    for i in range(j+1, k):
        #print(i)
        _, _, phone = custom_line_split(lines[i])
        #print(phone_str,"inside loop", phone)
        phone_str += encode_phone(phone)
        #print(phone)
    #print("before last custom", len(lines), k)
...
def build_word_FA_files(phone_FA_file_list, segmented_file, out_word_file):
    ...
    with codecs.open(segmented_file, encoding="UTF-8") as s:
        lines = s.readlines()
    #print(segmented_file, out_word_file)
    ...
    with codecs.open(out_word_file, encoding="UTF-8", mode='w') as w:
        ...
        for i, line in enumerate(lines):
            ...
            for k, c in enumerate(line):
                ...
                if c == ' ' or c == '\n':
                    try:
                        #print(phone_FA_file_list[i], j, k - 1 - space_counter)
                        #print("entering collapse")
                    ...
                    w.write(name.split(".")[0] + ' ' + word_FA_line.replace("SIL","") + '\n')
                    #w.write(name[:-4] + ' ' + word_FA_line + '\n')
                    #print(name.split(".")[0] + ' ' + word_FA_line + '\n')
                    j = k - space_counter
                    space_counter += 1
