...
pushd evaluation/mboshi_ZRC_eval/
# RUN NAME TO MODIFY
RUN_NAME="330_gr_eval"
mkdir -p $RUN_NAME
pushd ./$RUN_NAME
LABELS_LIST=$1
SEGMENTED_LIST=$2
...
# 0. pre-process
EVAL_SCRIPT='/home/getalp/zanonbom/attention_study/evaluation/ZRC_corpora/mboshi/330/graphemic/eval2.py'
...
for i in $(cat $LABELS_LIST); do 
    ...
    cat $i | awk '{print $3, $1, $2}' > ./$FA_DIR_NAME/$bare_name.txt; 
...
CORPUS='mboshi' python $EVAL_SCRIPT -v $OUT_DIR_NAME.word.fa.classes  $OUT_DIR_NAME.eval/ -m boundary token/type
